...
function DeckGLCanvas({ plotterPoints, imageCount, xGap, yGap }) {
  ...
  return (
    <div style={{ position: "relative", height: 550, background: "#16213e" }}>
      ...
      />
      {tooltipInfo && <DeckGLTooltip tooltipInfo={tooltipInfo} />}
    </div>
  );
}
...
function createAxesLayers(plotterPoints, xGap, yGap) {
  ...
  for (let i = 0; i <= tickCount; i++) {
    ...
    texts.push({
      position: [scaledX, (yMax + padY + 2) * ySpacingScale],
  ...
  for (let i = 0; i <= tickCount; i++) {
    ...
    texts.push({
      position: [(xMin - padX - 2) * xSpacingScale, scaledY],
...
function PixiCanvas({ plotterPoints, imageCount, xGap, yGap }) {
  ...
  useEffect(() => {
    if (!isAppReady || !contentLayerRef.current || !axesLayerRef.current)
      return;
    if (plotterPoints.length === 0) return;
  ...
  const handleWheel = useCallback((event) => {
    ...
    const newScale = Math.max(
      ZOOM_MIN,
      Math.min(scaleRef.current * scaleDelta, ZOOM_MAX),
    );
...
function drawGrid(
  ...
  for (let i = 0; i <= AXIS_TICK_COUNT; i++) {
    const xValue =
      xExtent[0] + (xExtent[1] - xExtent[0]) * (i / AXIS_TICK_COUNT);
  ...
  for (let i = 0; i <= AXIS_TICK_COUNT; i++) {
    const yValue =
      yExtent[0] + (yExtent[1] - yExtent[0]) * (i / AXIS_TICK_COUNT);
...
function DeckGLCanvas({ plotterPoints, imageCount, xGap, yGap }) {
  ...
  const clipId = useMemo(
    () => `deckgl-clip-${Math.random().toString(36).slice(2)}`,
    [],
  );

  const { bitmapItems, hoverBoxes } = useMemo(() => {
    const items = [];
    const boxes = [];
    const scale = transform.scale;
    const contentOffsetX = PLOT_MARGIN.left + transform.x;
    const contentOffsetY = PLOT_MARGIN.top + transform.y;
    ...
    for (const point of normalizedPoints) {
      ...
      positions.forEach((position, index) => {
        const left = contentOffsetX + position.x * scale;
        const top = contentOffsetY + position.y * scale;
    ...
    return { bitmapItems: items, hoverBoxes: boxes };
  }, [normalizedPoints, baseXScale, baseYScale, imageCount, transform]);

  const layers = useMemo(() => {
    return bitmapItems.map(
      (item) =>
        new BitmapLayer({
          id: `bitmap-${item.id}`,
          image: item.image,
  ...
  const handleWheel = useCallback(
    (event) => {
      ...
      setTransform((prev) => {
        ...
        const nextX = prev.x - (localX - prev.x) * (nextScale / prev.scale - 1);
        const nextY = prev.y - (localY - prev.y) * (nextScale / prev.scale - 1);
        return clampTransform(
  ...
  const handlePointerMove = useCallback(
    ...
    [hoverBoxes, innerWidth, innerHeight],
  );
